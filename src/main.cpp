...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PixelCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PixelCoin Signed Message:\n";
...
static const int64 nTargetTimespan = 4 * 60 * 60; // PixelCoin: every 4 hours
static const int64 nTargetSpacing = 60; // PixelCoin: 1 minutes
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal pixelcoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "PixelCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // PixelCoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
